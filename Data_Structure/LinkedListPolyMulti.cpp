...
struct node4 *start4=NULL,*l4=NULL;
...
struct node4* getNode4()
{
	struct node4 *t=(struct node4*)malloc(sizeof(struct node4));
	t->coef=NULL;
	t->exp=NULL;
	t->next=NULL;
	return t;
...
void multi()
{
	struct node1 *p=start1;
	struct node2 *q=start2;
	struct node4 *s=start4;
    	while(p!=NULL && q!=NULL)
	{
		struct node4 *temp=getNode4();
		if(p->exp == q->exp)
		{
			temp->coef=p->coef * q->coef;
